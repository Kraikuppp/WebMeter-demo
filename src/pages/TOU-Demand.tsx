...
export default function TOU_Demand() {
  ...
  const prepareChartData = () => {
    ...
    // สำหรับ Day view - สร้าง time bucket ทุก 15 นาที เริ่มจาก 9:00
    let timeBuckets = [];
    let t = 9 * 60; // เริ่มจาก 9:00 (540 นาที)
    while (t < 24 * 60) {
      timeBuckets.push(t);
      t += 15; // เปลี่ยนจาก 60 เป็น 15 นาที
    }
    // เพิ่มช่วงเวลาหลังเที่ยงคืน (1:00-8:45)
    ...
    for (const targetMinute of timeBuckets) {
      ...
      const timeWindowEnd = targetMinute + 15; // เปลี่ยนจาก 60 เป็น 15 นาที
      ...
      if (targetMinute < 9 * 60) {
        // ช่วงหลังเที่ยงคืน (1:00-8:45) ให้เป็น 25-32.75
      ...
      chartData.forEach((row, rowIndex) => {
        ...
        displayColumns.forEach(column => {
          ...
          if (rowIndex < 3) { // แสดง log เฉพาะ 3 แถวแรก
            console.log(`     Row ${rowIndex}: time=${timeValue}, ${key}=${value}, isOnPeak=${isOnPeak}`);
          }
